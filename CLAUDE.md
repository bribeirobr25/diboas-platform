...
- **Lazy-loaded calculator pattern (Phase 6C.0/2026-05-11):** `CompoundInterestCalculator` is consumed only via `dynamic(() => import('@/components/Sections/CompoundInterestCalculator'), { ssr: false })` from each tool route (`/tools/compound-interest`, `/tools/retirement`, `/tools/goal-savings`) AND the lesson route (`/learn/compound-interest` via `LessonThreeBeat`). Never static-import it — keeps every consumer within bundle-budget caps. The component exports both a named function and a default for dynamic-import compatibility (knip flags it as a duplicate; that flag is expected).
