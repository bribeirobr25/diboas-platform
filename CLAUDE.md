...
- **BotID Stage 2 enforcement** — founder go after the clean observation week, then the Stage 3 Deep Analysis toggle.
- Add the `SNYK_TOKEN` repo secret (app.snyk.io → Account settings) to arm the quarterly `snyk test` step.
- Sync diboas-analytics.com German (Sie → du) — the one remaining DE decision (Sie otherwise stays ONLY on legal ×3 + the investor vertical; do-not-regress).
- DMARC (5.34) mailbox re-eval — overdue.
