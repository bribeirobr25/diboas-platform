...
diboas-platform/
  ...
  apps/sandbox/        # Play-money B2C+B2B practice app (work-front; docs/sandbox-app/) —
                       #   real data/integrations except custody; gated + noindex. Uncommitted WIP.
...
**In flight 2026-07-21→22 (sandbox launch-readiness + Stage-1 build — VALIDATED GREEN, UNCOMMITTED; founder commits everything together):**

- **W1/W2/W3 + B2 GENIUS wall — DONE + staged (web 1,334 tests):** W1 sandbox tagline honesty; W2 `/strategies` Case-4 reframe (advice copy neutralized ×4); W3 web Money Jobs → two-bucket (Essentials + Working, Emergency-Fund = first job); B2 the **US GENIUS "stablecoin never adjacent to earn/yield" wall** — reframed strings + the build-breaking `apps/web/src/lib/i18n/__tests__/genius-stablecoin-wall.test.ts` guard (allow-list CLO-scoped) + a sandbox mirror guard. CLO signed off; the 3 covenants (GENIUS wall · BR-top-up no-incentive · geofence) are do-not-regress.
- **Legal/counsel round:** counsel brief + the CLO "if I were your lawyer" answers → founder relayed EU/US/BR **lawyer feedback** (cleared under covenants; _licensed-opinion status TBC — B3 formal opinions remain the record-gate_). CLO rulings R-1…R-6 landed (R-5 relaxed: **server-side financial profile OK under 5 non-severable conditions**; anonymous-play-first mandatory; 18+ self-attestation; deletion cascade; consent versioning; **four unbundled opt-ins**). **Best-practice ToS/Privacy/M2 DRAFTS in `docs/sandbox-app/legal-drafts/` ×4 langs** (for counsel to red-line, NOT legal advice).
- **W6 Stage-1 auth foundation — plan + BUILD started** (`SANDBOX_W6_STAGE1_PLAN_2026-07-21.md`, from a 4-stream deep read; §17 = AS-BUILT + audit + PAUSE/RESUME). **BUILT + validated (41 sandbox tests):** M1 **geofence** (`apps/sandbox/src/middleware.ts` + `config/geofence.ts` + `/unavailable` 451, CN/RU/KP, verified 451 live) · the **`IAuthProvider` seam** (`lib/auth/*`, `SimulatedAuthProvider` wraps the gate, behaviour-preserving layout swap). **DEC-7=B** (copy security utils into the sandbox, not extract — extract-to-`@diboas/security` is a `PENDING_ALL` follow-up). **Paused after Phase 0 + P1.1** — the rest (Neon DB + `PostgresLedgerStore` + anon→account migration; Auth.js + 4 methods; consent/18+/ToS; hardening) is **infra/config/counsel-text-gated** (sandbox Neon + deps + OAuth/Resend + counsel text). `apps/web` untouched by the sandbox build. Next CC step when the DB/deps land: W6 P1.2 (migrations + tables). Middleware→proxy Next-16 deprecation = repo-wide follow-up.

**In flight 2026-07-18→20 (multi-day UX-research + sandbox session — VALIDATED GREEN, mostly UNCOMMITTED on `main`; founder to commit):**

- **Sandbox app** (`apps/sandbox/` + built-out `packages/banking|defi|investing`) — B2C+B2B play-money practice app: event-sourced ledger + Decimal.js fee math, strategy catalog-as-data, provider interfaces (market data real; custody simulated), reconciliation invariant. Full mobile-app redesign + a v2 UX pass. Gated + noindex. Work-front docs in `docs/sandbox-app/` (rules, build order, redesign plan, `MVP0_VALIDATION_LOG.md`, CMO eval). Promotion gates (real auth, pa11y/Lighthouse/security CI, es/de transcreation) deferred + recorded in the build-order doc.
- **UX-governance research integration** (`docs/audit/UX_RESEARCH_INTEGRATION_PLAN_2026-07-13.md`) — canon grew **Part 9 (UX-60…63, honest data-visualization)** + **Appendix E (regulatory anchors R-1/R-2/R-3, CLO-verified Session 023, D-2 LIGHT = supporting authority not binding gate)**; design-reviewer gained a data-viz-honesty phase; Appendix B rows annotated (R-4 Robinhood, Princeton academic); VOICE_RUBRIC Q3 points to the anchor. Governance wiring fix: `asset-compliance-checklist.md` + `STORYTELLING_CRAFT.md` now under `validate:ux-canon`; retention extended to `docs/socials/avatar/`. Canon now **UX-01…63**. Founder ruled option B: R-5…R-12 stay recorded-verified authority in `docs/audit/CLO_BOARD_R_REGISTER_VERIFICATION_2026-07-20.md`, not canonized.
- **Web-app UX fixes from the review** — Dream Mode data-viz honesty (0-decimals, assumed-APY label, scenario spread + caveat per UX-60/63); `/market` server-side outage banner; language selector flags→endonym labels; Dream Mode timeframe i18n label + locale-correct day separators; sandbox goal-icon a11y labels (F-1) with a guard test.
- Full record: `docs/audit/UX_RESEARCH_IMPLEMENTATION_REVIEW_2026-07-19.md`.
...
- ~~`/investors` form end-to-end test + error-volume check~~ ✅ **DONE 2026-07-17 PM (Claude, live E2E): the pipeline works** — test submission → 200 + success UI; Vercel runtime logs show only 1 request in 7 days (the test itself) → no investor contacts lost in that window. **Forensic CORRECTION found en route: `apps/web/.env.local` `DATABASE_URL` is STALE** (points to an old Neon DB whose data ends 2026-07-11) — the founder's morning Neon-console fix was the operative one; founder to refresh `.env.local` from Vercel before any local DB runbook. Also found: first submit on a cold DB 500s after ~30 s (Neon scale-to-zero; retry succeeds — logged as a pending item). Notification email CONFIRMED received (founder). **Follow-up shipped same day: notification now sets Reply-To = requester (answering = plain Reply) + `pnpm investor:read` decrypt CLI (runbook reader for the encrypted records; 3 contract tests).** Remaining micro-steps: delete the test row; refresh `.env.local` `DATABASE_URL` from the NEON dashboard (Vercel env values are write-only sensitive — not recoverable from Vercel; 5.72). Full record: incident-doc addendum 2026-07-17 PM. ~~Search Console sitemap resubmit + Request Indexing~~ ✅ DONE (founder, 2026-07-17); ~~How-it-Works section visual + prod spot-check~~ ✅ DONE (founder-verified 2026-07-17, new editorial trio live).
- **Snyk batch 2026-07-17 (F24) handled:** postcss CVE-2026-41305 FIXED via targeted override `postcss@8.4.31→8.5.16` (build + 1,329 tests green); esbuild SNYK-JS-ESBUILD-17750822 ACCEPTED not-exploitable (Deno-only path; no CVE — invisible to `pnpm audit`, the F24 blind-spot lesson; re-check 5.73). Audit baseline now: prod CLEAN, full 1 mod/1 low (dev-only). **Founder: add the `SNYK_TOKEN` repo secret** (app.snyk.io → Account settings) to arm the new quarterly `snyk test` step.
