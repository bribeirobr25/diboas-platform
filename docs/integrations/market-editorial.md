...
apps/web/data/market/shared/
  ...
  historical.json             # since-inception weekly series of past regime scores (one point per real run)
...
**State-grammar copy (`/market/backdrop`, 2026-08-24).** The backdrop lead is NOT `groups[].summary` (that one keeps its "(N of M points)" for the scored Bitcoin view). It is generated separately into `groups[macro_environment].state_view` from `apps/web/scripts/market-refresh/templates/state-beats.json`. **The beats report CHANGE, not level:** each condition selects a `moved*` or `hold*` variant by comparing its state to the PREVIOUS RUN DAY in `run-archive.jsonl`, and falls back to `hold` when there is no prior run. Never rewrite a beat to describe a level ("the dollar has slipped below its trend") — it would imply a recency the data does not have, every week. Never add a duration ("for six weeks"): the archive starts 2026-07-11, so any duration is a floor. Editorial overrides work here too, keyed `stateView.lead.<locale>` / `stateView.depth.<locale>`.
...
| `signals.json` → `groups[macro_environment].state_view` | **`/market/backdrop` only** (added 2026-08-24): the state-grammar `lead` (points-free opening + one beat per condition) and `depth` (the numbers, behind a toggle). GENERATED from `templates/state-beats.json`; `--check` rejects a hand-edit. The scored views ignore it. |
...
| `historical.json`         | **Pipeline-owned — never edit by hand**     | `generate.mjs` appends one point per real run and flips `synthetic_seed`. Hand-adding a point publishes history that never happened (5.127). A 52-entry cap still exists in code and will start dropping the oldest points around mid-2027, at which point the "since the framework went live" copy needs revisiting — tracked as **5.139**. |
...
| `summary`          | Object keyed by locale (`en`, `pt-BR`, `es`, `de`) — see §5 below for the translation workflow. Each locale has `short`, `detailed`, `mixed_signals` (boolean), `key_supportive_factors` (array of strings), `key_headwinds` (array of strings). **`confidence_level` is NO LONGER editorial** — since 2026-08-24 (5.131) the generator derives it from `data_status.overall_confidence` and `--check` rejects a hand-edit.                                                                                                             |
...
| Field            | Constraint                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `range`          | `"since-inception"` — the series starts when the framework went live (2026-07-11), NOT a rolling year                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `snapshots`      | One entry per REAL pipeline run — `date` (ISO date), `score` (integer 0–14), `regime_code` (the 5-band enum). **There is no minimum count, and you must never add one.** Every entry must correspond 1:1 to a distinct `run_at` day in `run-archive.jsonl`; the guard is `fixtures.test.ts` → "should publish only snapshots backed by a real archived run". A length floor is what made inventing history the only way to go green (5.127) — the old `>= 50` assertion is deleted, do not reinstate it here or in code. **Only the pipeline appends; never hand-add a point.**                             |
| `synthetic_seed` | Boolean, **generator-owned — never hand-edit it** (`generate.mjs#nextHistorical`, which flips it to `false` once `run-archive.jsonl` holds >= 8 distinct real run DAYS). When `true`, the trend-chart section on `/market` is suppressed via `!historical.synthetic_seed`. **A hand-flip of this flag is the documented root cause of the 2026-08-24 live defect** (commit `37b23b20` set it `false` while 44 illustrative seed points were still in the file, publishing seed as measured history — 5.127/5.27). The flag exists so it can never outrun the data; editing it by hand defeats exactly that. |
...
> **⚠️ HISTORICAL — do not follow this as a procedure (marked 2026-08-24).** Everything in this
> subsection describes the pre-P4 world, when ETF-01 was read by hand from CoinGlass/Farside and an
> editor wrote the verdict. There is no manual ETF-01 step any more: `resolveEtfSignals()` in
> `apps/web/scripts/market-refresh/lib/etf-flows.mjs` is the single route for BOTH entry points
> (pipeline and the `compute-regime.mjs` check), reading the Polygon shares-outstanding ledger and
> falling back to `etf01-manual.json` only when that ledger is empty. Leaving this readable as live
> procedure is how `compute-regime.mjs` went two months contradicting the published score (5.136).
> Kept for the §4.5-vs-§10.1 reasoning, which still governs how the ENGINE scores a gap.
