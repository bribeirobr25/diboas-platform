...
**Audit purpose:** Specify _what_ each tool computes (outcome contract) without prescribing _how_ (algorithm). An auditor implementing each tool from first principles, using any reasonable formula choice, should converge on the same numeric results documented in `TEST_VECTORS.json`.
...
| Effective rate `(1 + r_S)(1 + d_locale) − 1` would be ≤ −0.99   | Floor-clamp to −0.99 (≈ −99% APY) so the downstream geometric monthly conversion `(1 + r)^(1/12) − 1` stays finite-defined. Emit observability event (see below). Reachable only when `d_locale ≤ −1` (local currency appreciating ≥ 100% — not seen in current data; safeguard for future SDK responses). |

> **Floor-clamp observability (added 2026-05-26, post-CTO-board v3 audit):** when the effective-rate clamp activates, the calling site MUST emit `applicationEventBus.emit('tools:calculatorDepreciationClamped', { source, usdYield, depreciation, clampedTo: −0.99 })` so the silent rewrite is auditable. Implementation is the single helper `applyEffectiveRateClamp` in `apps/web/src/lib/market-data/formulas/currencyHedge.ts`, consumed by all 5 hedge sites (FV-shape `calculateWithCurrencyHedge`, annuity-shape `calculateMonthlyWithCurrencyHedge`, compound engine `calculateCompoundProjectionHedged`, `lib/emergency-fund/calculator.ts`, `lib/time-to-target/calculator.ts`). Auditors verifying the clamp can grep for `applyEffectiveRateClamp` to confirm coverage.
...
Answer: _"If I save a small amount on a regular schedule for several years, what would my money become at different investment-return assumptions?"_
...
Answer: _"If I contribute every month for 25 years, what would I have at retirement?"_
...
The de Retirement default (€400/mo × 25y) must produce a Historical FV of **€541,891 ± €20** (**PT3'**, re-signed Bar 2026-05-26 per FX-16 adoption D1+D2). The retired prior PT3 €608,815 reflected the now-corrected EUR field-placement error (1.23% endpoint-pair retrospective CAGR mistakenly placed in the forward `annualDepreciation` field; now lives in `historicalCagr` only).
...
_"How much would I have if I saved $X/month for Y years toward a goal?"_
...
_"How many months will it take me to save up an emergency fund of N months of expenses, given my monthly savings rate?"_
...
_"How long until I reach $X if I contribute $Y/month?"_
...
_"What if I had invested $X in [asset] starting in [year], either as a one-time amount or as $X/month DCA?"_
...
> **Output**: the present-day value of `amount` invested as a single lump sum at the start of the user's chosen `startYear` month, valued at the most recent month's closing price — denominated in the user's `displayCurrency` (default `USD`).
>
> **Single-currency case** (`displayCurrency` equals the asset's native pricing currency — see §6.7 native-currency table):
> `terminalValue = amount × (finalPrice / startPrice)`
>
> **Cross-currency case** (`displayCurrency` differs from asset-native, e.g. pt-BR user buying USD-priced SP500):
> `amountInAssetCcy = amount × fxLookup(displayCurrency → assetCcy, startMonth)`
> `terminalInAssetCcy = amountInAssetCcy × (finalPrice / startPrice)`
> `terminalValue = terminalInAssetCcy × fxLookup(assetCcy → displayCurrency, finalMonth)`
>
> The `fxLookup` function reads `closeLocalPerUsd` from `monthlyFx[targetCurrency].months[].ym` (forward-fills to latest available `ym ≤ requested` if the exact month is missing). Cross-rates compose via USD as the pivot currency.

Where:

- `startPrice` = `closePriceOnly` field if `returnsBasis = 'price_only'` AND the asset has the field populated; otherwise `close` field (the asset-native series remains the unit-of-account regardless of `displayCurrency`)
- `finalPrice` = same field selection at the most-recent month
- `startMonth` = the FIRST AVAILABLE month in `startYear` from the asset's data series (data-driven; see §6.10 for the below-data-floor rule). NOT a fixed July/January assumption — **post v1.4 CoinMetrics backfill**: BTC begins July 2010; DAX begins June 2010; SP500/QQQ/GOLD/TLT/IBOVESPA begin July 2010; MSCI_WORLD begins January 2012 (URTH ETF inception). The calculator finds the first row whose year matches `startYear`.
- If no row exists in `startYear` at all (e.g. `MSCI_WORLD` with `startYear ∈ {2010, 2011}` — the only sub-2012 floor remaining post-backfill), the tool throws `AssetHistoryDataError` and the UI displays no result panel (silent graceful per explicit decision, no crash). All BTC `startYear ∈ {2010, …, 2026}` now succeed post-backfill.

### 6.5 Outcome contract — Monthly DCA

> **Output**: the present-day value of `amount` invested at the end of each month from the first available month in `startYear` through the most recent month in the data series, where each contribution buys a fractional position at that month's `close` price in asset-native currency, and the resulting fractional positions are valued at the most-recent month's `close` price — denominated in the user's `displayCurrency` (default `USD`).

Mathematically: if `closeₘ` denotes the close at month `m` in the window (in asset-native currency), there are `M` months in the window, and `α_m = fxLookup(displayCurrency → assetCcy, ym_m)` is the month-`m` FX conversion factor (1.0 in the single-currency case):

> `terminalValue = (Σₘ₌₁ᴹ (amount × α_m) / closeₘ) × close_M × fxBack`

where `fxBack = fxLookup(assetCcy → displayCurrency, ym_M)` (1.0 in the single-currency case).

In the single-currency case (`displayCurrency` = asset-native) this reduces to:

> `terminalValue = (Σₘ₌₁ᴹ amount / closeₘ) × close_M`

Additionally, the **range output** is computed for entry-timing sensitivity:

> `rangeHigh = (Σₘ₌₁ᴹ (amount × α_m) / effLowₘ) × close_M × fxBack` (best case: each contribution bought at the month's low)
> `rangeLow  = (Σₘ₌₁ᴹ (amount × α_m) / effHighₘ) × close_M × fxBack` (worst case: each contribution bought at the month's high)

where `effHighₘ` and `effLowₘ` are the basis-consistent high/low per §6.11 (factor-adjusted into TR space when `closePriceOnly` is present and `returnsBasis = 'total_return'`).

The PT2 toggle: if `returnsBasis = 'price_only'`, replace `closeₘ` with `closePriceOnlyₘ` (if populated) in all three sums and in `close_M`; in price-only mode no factor adjustment is applied to `effHigh`/`effLow` (they remain the raw `high`/`low`).

### 6.6 Confidence stratification (clarified v1.7 per Auditor 4 G2)

Output a `confidence` label per the M6 calm-framing principle:

- BTC + `startYear ≤ 2012` → **LOW**
- BTC + `startYear ≥ 2013` → **MEDIUM**
- All other (asset, startYear) combinations → **HIGH**

**Important distinction — calculator return shape vs. UI display:**

- The **calculator** ALWAYS returns the full output shape per §6.5 (`terminalValue` + `rangeLow` + `rangeHigh` + `confidence` etc.) regardless of confidence label. This is what `TEST_VECTORS.json` captures and what audit implementations must reproduce.
- The **UI layer** uses the `confidence` label to choose how to _display_ the result:
  - **LOW** → renders the `[rangeLow, rangeHigh]` band only; the central `terminalValue` is computed but visually suppressed because point-estimate precision is misleading for BTC 2010-2012's volatility.
  - **MEDIUM** → renders `terminalValue` with the explicit ± range bar.
  - **HIGH** → renders `terminalValue` with an implicit ± entry-timing range underneath.

Auditor implementations should reproduce the full output shape (terminalValue + range bounds + confidence) for every scenario, including LOW-confidence rows like `assetHistory_13`. The UI suppression of `terminalValue` for LOW is a render-layer choice, not a calculator-output contract.
...
| `displayCurrency` | enum (optional) | en:USD, pt-BR:BRL, es/de:EUR | {USD, BRL, EUR}; defaults to USD when omitted |

**Output fields (defined v1.7 per Auditor 4 G3):**

| Field                    | Type                            | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------ | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `terminalValue`          | number                          | Present-day value per §6.4 / §6.5. Always present, including when `confidence === 'LOW'` (the LOW label governs UI display, not calculator return — see §6.6).                                                                                                                                                                                                                                                                                                                                                      |
| `rangeLow` / `rangeHigh` | number                          | Entry-timing range per §6.5 / §6.11. Always present for `mode='monthlyDca'`; absent for `mode='lumpSum'` (no entry-timing uncertainty).                                                                                                                                                                                                                                                                                                                                                                             |
| `months`                 | integer                         | Number of months in the DCA window (or holding period for lump-sum).                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `totalContributed`       | number                          | For DCA: `amount × months` in `displayCurrency`. For lumpSum: `amount`.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `confidence`             | enum {HIGH, MEDIUM, LOW}        | M6 calm-framing label per §6.6.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `basis`                  | enum {total_return, price_only} | **Echo of the `returnsBasis` INPUT** — confirms which basis was used for the computation. NOT a statement about whether the asset's underlying data series carries dividends (BTC's underlying data is price-only since BTC has no dividends, but the calculator still reports `basis: 'total_return'` because that was the requested input mode; for BTC the two modes produce identical results since the asset has no dividends to reinvest). Auditor implementations should echo the `returnsBasis` input here. |
| `startYm` / `endYm`      | string (YYYY-MM-DD)             | First and last month of the window. Useful for verifying §6.10 below-data-floor rule.                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `displayCurrency`        | enum                            | Echo of the input field (or `'USD'` default). Confirms cross-currency math applied.                                                                                                                                                                                                                                                                                                                                                                                                                                 |

**`displayCurrency` semantics (v1.5, 2026-05-23):** the currency in which the user's `amount` is denominated AND in which the terminal value should be reported. The UI derives this from the user's locale (en → USD, pt-BR → BRL, es/de → EUR) and passes it to the calculator. When it equals the asset's native pricing currency (see §6.7 — USD for BTC/SP500/QQQ/MSCI_WORLD/GOLD/TLT; BRL for IBOVESPA; EUR for DAX), the single-currency math in §6.4/§6.5 applies (no FX conversion). When it differs, the cross-currency math applies: each contribution converts to asset-native at THAT month's `closeLocalPerUsd` from `monthlyFx`, the unit math runs in asset-native, and the terminal converts back to `displayCurrency` at the end-month's rate. Cross-rates (e.g., pt-BR user buying DAX = BRL→EUR) compose via USD: `BRL → USD → EUR`. Forward-fill handles end-of-window gaps (e.g., ECB EUR data may lag asset-price data by 1 month — uses the latest available `ym ≤ requested`).
...
| BTC 2010 DCA $100/mo (post-v1.4 backfill) |            terminalValue $534,529,757; range [$461,501,201, $718,288,426]; 191 months; confidence LOW | ±$1000 on terminal; ±$1000 on range bounds |
| BTC 2014 DCA $100/mo (post-v1.4 backfill) | terminalValue $811,847; range [$709,487, $931,927]; 149 months; confidence MEDIUM; startYm 2014-01-01 | ±$50                                       |

### 6.10 Below-data-floor `startYear` handling (A2 fix, 2026-05-23; rewritten v1.2 per Auditor 4 N2; rewritten again v1.7 per Auditor 4 G1 — post-v1.4 BTC backfill landed)

Each asset's monthly series has a first-available-month: **BTC begins July 2010** (post v1.4 CoinMetrics backfill spliced with Yahoo from 2014-09 onward — see §6.7); MSCI_WORLD via URTH begins January 2012; the remaining 6 assets begin July 2010 (DAX begins June 2010). MSCI_WORLD is the only asset that still has a sub-2012 floor.
...
In plain language: **find the first row whose year matches the requested `startYear`. If at least one row exists in that year, succeed and use that row as the start; if no row exists, throw an error.** The implementation does NOT special-case "below floor year" vs "at floor year" — there is a single uniform rule, and the outcome falls out from whether the requested year has any data at all. The rule itself is unchanged from v1.2 — only the _per-asset table below_ changed in v1.4 when BTC's data range expanded.

Concrete behavior per asset (post-v1.4 reality):

| Asset (first available month)                 | `startYear=2010`                   | `startYear=2011`  | `startYear=2012`  | `startYear=2014`                   |
| --------------------------------------------- | ---------------------------------- | ----------------- | ----------------- | ---------------------------------- |
| BTC (2010-07, post-backfill)                  | **success — 2010-07** (191 months) | success — 2011-01 | success — 2012-01 | **success — 2014-01** (149 months) |
| MSCI_WORLD (2012-01)                          | error                              | error             | success — 2012-01 | success — 2014-01                  |
| SP500 / QQQ / GOLD / TLT / IBOVESPA (2010-07) | success — 2010-07                  | success — 2011-01 | success — 2012-01 | success — 2014-01                  |
| DAX (2010-06)                                 | success — 2010-06                  | success — 2011-01 | success — 2012-01 | success — 2014-01                  |

Test vectors `assetHistory_13` (BTC `startYear=2010`) and `assetHistory_14` (BTC `startYear=2014`) reflect this rule under the post-v1.4 data:

- `_13`: succeeds with `startYm=2010-07-01`, 191 months, `terminalValue=$534,529,757`, `confidence=LOW` (per §6.6).
- `_14`: succeeds with `startYm=2014-01-01`, 149 months, `terminalValue=$811,847`, `confidence=MEDIUM`. **No fallback to 2014-09 happens** because the 2014-01 row exists in the post-backfill series.

**Legacy code path retired (v1.4) and DELETED (2026-05-26, `TOOLS_41_DEFECTS_FIX_PLAN.md` §2.7 / C19 close):** the older `calculateAssetHistory` (anchor-table-based, LOW-confidence range $500M–$1.5B for BTC 2010 DCA $100/mo derived from research) was a fallback used while the DCA replay path errored for BTC pre-2014. Post-v1.4 the replay path produced a real $534.5M result that sits inside the legacy research envelope — the legacy path was no longer exercised by `/tools/asset-history` for any BTC startYear. **2026-05-26 update:** the legacy `calculateAssetHistory` function, `DCA_TERMINAL_PER_100`, `monthsBetween`, `pairConfidence`, `AssetHistoryArgs`, and `AssetHistoryResult` were physically removed (~120 LoC). The current `apps/web/src/lib/asset-history/calculator.ts` exports ONLY `calculateAssetHistoryDcaReplay` and `calculateAssetHistoryLumpSum`. Auditors verifying behavior should ignore any historical references to `calculateAssetHistory` — the function no longer exists.
...
Forward mode: _"What will today's $X be worth in N years, given expected inflation?"_
Retrospective mode: _"What would $X from 2010 be worth today, given cumulative inflation?"_
...
_"What does holding cash in [non-USD currency] cost over N years compared to bank yield or diBoaS digital-dollar return?"_
...
_"How much do card-processor fees cost my business per year, and how much could I save with diBoaS?"_
...
_"What could my business's idle cash earn at current bank rates vs at diBoaS Conservative yield?"_
...
5. **Spot-check the PT acceptance gates** (PT1 R$7,336,100; PT2 SP500 TR vs price-only ~18.33% delta — was 18.76% pre-F1 dedup; **PT3' €541,891** — re-signed Bar 2026-05-26 per FX-16 D1+D2, was €608,815 pre-FX-16 adoption; BTC_RECON $261,202 — was $254,188 pre-F1 dedup). These are Bar-signed product-truth values and must be reproduced exactly.
