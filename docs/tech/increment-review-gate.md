...
| E6  | **Accessibility, measured not assumed:** contrast as a resolved ratio (composited opacity included), touch targets **≥44×44 CSS px** with spacing — the handoff §6.6 floor; `24×24` is ONLY a lower bound for narrow legitimate exceptions such as inline text links in prose (⚑ corrected 2026-09-14: this row said a bare "≥24px", which the pre-I2 audit flagged as contradicting the ruled 44×44. CLAUDE.md and SANDBOX_RULES still say 24 — that conflict is `5.234`, a founder ruling, and is NOT resolved here; this row now cites the handoff, which is the later authority for this app), no heading-level skips, focus-visible, `prefers-reduced-motion` respected. pa11y catches none of the middle three. |
