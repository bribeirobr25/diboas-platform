...
- **Triggers:** weekly cron (**Mondays 09:23 UTC**, ~3h after the refresh window) + manual `workflow_dispatch`.
...
- **Why:** on 2026-08-31 GitHub dropped the weekly schedule entirely — workflow active, cron intact, another scheduled workflow in the same repo fired that morning. Nothing noticed. `/market` would have aged to its 14-day staleness gate with no signal (PENDING_ALL 5.151).
...
| **Market refresh watchdog** | Weekly    | `23 9 * * 1` (Mon 09:23 UTC)                             | GitHub `market-refresh-watchdog.yml`       | Asserts a Market Refresh run STARTED in the last 24h. Read-only (`actions: read`); never touches data or opens a PR. Exists because GitHub silently dropped the 2026-08-31 schedule and nothing noticed (PENDING_ALL 5.151). | Red run = the refresh did not fire; remedy printed in the error (`gh workflow run market-refresh-weekly.yml --ref main`). Known limits: a manual dispatch also satisfies it, and the watchdog is itself a schedule GitHub could drop. |
