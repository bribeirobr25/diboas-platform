...
Eight workflows in `.github/workflows/`. Five run on push/PR (`ci.yml`, `security.yml`, `accessibility.yml`, `e2e.yml`, `lighthouse.yml`); three run only on a schedule (`market-refresh-weekly.yml`, `market-refresh-watchdog.yml`, `security-scan-quarterly.yml`). `security.yml` runs on both. For the consolidated cross-system view of every recurring job (including the Vercel cron), see **Scheduled jobs — every cadence in one place** at the end of this section.
...
- **Triggers:** weekly cron (**Mondays 06:17 UTC**) + manual `workflow_dispatch`. _(:17 not :00 — GitHub drops scheduled runs at popular cron minutes; the 2026-08-31 run never fired at all. PENDING_ALL 5.151.)_
...
- **Output:** opens a PR on branch `editorial/market-refresh-auto` (labels `market-refresh`, `needs-editorial-review`, plus `memo-rewrite-required` when the analyst memo quotes figures this cycle no longer produces — that PR is titled `MEMO REWRITE REQUIRED` and its CI is red until a human updates the memo ×4 locales). It **never pushes to `main`** — a human reviews the plain-language copy and merges. The 14-day staleness gate is the backstop; each Monday's PR self-resets, so unmerged weeks accrue no debt.
...
### `market-refresh-watchdog.yml` — dropped-run alarm (scheduled)

- **Triggers:** weekly cron (**Mondays 22:00 UTC**, deliberately late in the day) + manual `workflow_dispatch`. _(An earlier 09:23 slot assumed 1–2h delays; the 2026-08-31 run was **7h19m** late, which would have produced a false alarm. PENDING_ALL 5.151.)_
- **Does:** queries the Actions API for the most recent `market-refresh-weekly.yml` run and fails closed if none started within 24h. No checkout, no data, no PR; permissions are `actions: read` + `contents: read`.
- **Why:** on 2026-08-31 the weekly refresh did not run at its scheduled time — workflow active, cron intact, yet nothing at 06:00; it eventually started at **13:19 UTC, 7h19m late**, and nobody would have known either way. GitHub delays and sometimes drops scheduled runs under load. Unwatched, `/market` ages to its 14-day staleness gate with no signal (PENDING_ALL 5.151).
- **Deliberately checks "a run started", not "a run succeeded":** a refresh that runs and legitimately fails a data gate is the fail-closed pipeline working, and that already notifies.
...
Five recurring jobs run without human action, across two systems (Vercel cron + GitHub Actions). This table is the single source of truth for "what runs on a timer and why"; the per-workflow detail is above and (for the Vercel cron) in §2.
...
| **Market refresh**          | Weekly    | `17 6 * * 1` (Mon 06:17 UTC)                             | GitHub `market-refresh-weekly.yml`         | Regenerates `/market` regime data + editorial copy; opens a review PR (never pushes). Keeps the publicly-indexed weekly-cadence page honest without a manual chore.                                                                                                                                               | Red run on pipeline/gate failure; 14-day staleness gate is the backstop.                                                                                                                                                              |
| **Market refresh watchdog** | Weekly    | `0 22 * * 1` (Mon 22:00 UTC)                             | GitHub `market-refresh-watchdog.yml`       | Asserts a Market Refresh run STARTED in the last 24h. Read-only (`actions: read`); never touches data or opens a PR. Exists because the 2026-08-31 run started 7h19m late and nothing would have noticed either way (PENDING_ALL 5.151). Checked late in the day so an ordinary delay is not mistaken for a miss. | Red run = the refresh did not fire; remedy printed in the error (`gh workflow run market-refresh-weekly.yml --ref main`). Known limits: a manual dispatch also satisfies it, and the watchdog is itself a schedule GitHub could drop. |
