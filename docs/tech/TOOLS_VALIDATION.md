...
- **Formulas:** annuity FV, lump-sum FV, monthly-iterative time-to-target, purchasing power, horizon-matched currency-hedge CAGR, path-dependent FX walk, BTC DCA monthly replay, poupança regime-switch. All math reproduces the existing test suite (862/862 passing) and the live production renders.
- **Results:** verified against Bar-signed product-truth gates (PT1 R$7,336,100 reproduces exactly; PT2 toggle delta 18.33% for S&P 2010 DCA — within PT2 ±2pp gate; PT3 €608,815 reproduces exactly; BTC_RECON $261,202 — updated from $254,188 after F1 dedup fix). Edge cases produce expected behavior (1200-month cap → Infinity for unreachable targets; sentinel 0 on insufficient data; calm-framing range display for high-volatility BTC paths).
...
- **One real defect surfaced:** Asset History UI hardcodes start-month to July-for-2010 / January-otherwise, but BTC Yahoo data only starts September 2014. A user selecting BTC + 2010-2014 receives the legacy LOW-confidence range branch (correct) but selecting BTC + 2014 directly will throw `AssetHistoryDataError`. See §6 finding D-1.
...
| **happy-path / happy-* / longer-horizon / mode-comparison / phase-h-*** | Realistic user interactions: changing one input within typical ranges, comparing modes, exercising different scenarios. |
...
| **PT-acceptance / PT2-toggle / PT1-context** | Reproduce the Bar-signed decision-register magnitudes (PT1 R$7.34M, PT2 toggle, PT3 €608k). |
...
*Note: the pt-BR 0.29% effective rate at 1-year horizon is a sign of the horizon-matched-CAGR formula returning a near-zero BRL CAGR for the trailing 12 months (May 2025 → May 2026 USD/BRL was approximately flat). For longer horizons (25y), it correctly returns the 6.21% CAGR. **This is the documented "1-year window gets the most recent 12 months only" behavior** and is mathematically correct per the v1.1 §6.1 lock.*
...
**Value to users:** Concrete retirement-savings projection across 4 scenarios. The pt-BR/de/es eye-opening Historical numbers (R$7.34M / €608k / €608k) show the diBoaS-vs-bank delta scaled to a real lifespan.
...
| de (PT3 Bar-signed) | €400/mo × 25y | **€608,815** |
| es | €400/mo × 25y | €608,815 |
...
Both currently return **error**: `no data for BTC starting 2010-07` / `no data for BTC starting 2014-01`. BTC Yahoo data starts September 2014. The current calculator hardcodes startMonth as July for 2010 (data floor for non-BTC assets) and January otherwise. **This is finding D-1 in §10 below.** For BTC 2010 specifically, the legacy `calculateAssetHistory` path covers the LOW-confidence range (research-anchored $500M-$1.5B for $100/mo). The new `calculateAssetHistoryDcaReplay` path does not yet handle the BTC pre-Sep-2014 case.
...
| es | €80k | 2.0% | 4.31% (note: 3y EUR window has *negative* dep — EUR strengthened recently) | €84,897 | €90,787 | **+€5,891** |
...
| de Retirement €608,815 reproduces | ✅ PT3 Bar-signed |
...
### D-1 [Medium] — Asset History BTC pre-Sep-2014 start years throw error

**Tools affected:** `/tools/asset-history`
**Symptom:** Selecting BTC + 2010, 2011, 2012, 2013, or 2014 throws `AssetHistoryDataError: no data for BTC starting [year]-01` (or `-07` for 2010).
**Root cause:** `calculateAssetHistoryDcaReplay` searches for the first month of `startYear` (or July if 2010). BTC Yahoo data starts September 2014; earlier months don't exist in `monthlyPrices.json`.
**Severity:** Medium. The legacy `calculateAssetHistory` path (used by the original Phase E shipped surface) covers BTC 2010 via the research-anchored LOW-confidence range branch. But the new yearly-picker UI (Phase E v2) calls `calculateAssetHistoryDcaReplay`, which throws.
**Recommended fix (Phase E sub-PR):** In `calculateAssetHistoryDcaReplay`, for BTC + startYear ≤ 2014, fall back to the first available month (September 2014) instead of throwing. Adjust the result's `startYm` to reflect actual start. Alternatively, surface a UI "earliest available data: September 2014" hint when user selects an unsupported combination.
...
3. ✅ **`pnpm vitest run`** — full test suite passes (862/862 baseline)
4. ✅ **`pnpm validate:translations`** — all 4 locales in sync
5. ✅ **Stress-test simulator** — `pt-BR Retirement R$7,336,100` matches Phase A baseline ±2% (PT1 acceptance gate)
6. ✅ **Visual regression** — 14-page Docker MCP sweep, 0 console errors per page
7. ✅ **Decision register** — any PT-row touched is Bar-signed before the PR can merge
