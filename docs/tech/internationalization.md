...
- **Canonical namespace inventory** (currently **33 namespaces per locale** — the investor vertical added `investor` + `investor-docs`) + the Phase-7 Q4 banned-term grep gate: **`packages/i18n/README.md`**.
...
**Canonical builder pattern (omnibus map):** `apps/web/src/lib/market-data/feeComparisonValues.ts` exports `buildAllFeeValues(fees, locale)` returning a single `Map<string, Record<string, string>>` covering all migrated landing-page rate-citation keys. Consumers wrap it in `useMemo([locale])` and pass it to `useConfigTranslation`. Verified consumers: `FeeTable`, `ProseSection`, `FAQAccordionFactory`. Components with their own local `t` helpers that need rate values extend the helper with an optional `values?` parameter instead of taking an omnibus-map dependency (the former examples, `B2BGoalCards`/`GoalExampleCard`, were deleted 2026-07-07 — `CurrencyDepreciationCalculator`'s `t` follows the same pattern).
