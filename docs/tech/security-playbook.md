...
- Migrations 001-014 (`apps/web/src/lib/database/migrations/*.sql`; 014 = `investor_requests`) — schema-only; no seed PII; AES-256-GCM + HMAC blind index pattern correctly applied to `waitlist_entries` and `email_delivery_logs`. Migration 013 + the daily `purgeExpiredAuditLogs` cron enforce 90-day retention on the `audit_logs` forensic trail (raw `actor_ip` under legitimate interest — §5.39)
