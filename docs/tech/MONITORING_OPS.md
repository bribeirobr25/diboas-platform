...
Browser ─┬─ Sentry SDK ──POST──> /api/monitoring (same-origin tunnel) ──> *.ingest.sentry.io
         ...
         │
         └─ web-vitals   ──POST──> /api/monitoring (also tunneled through Sentry)
...
- The handler **deliberately skips `applyRateLimit` and `applyCsrf`** that other API routes use. Sentry posts on every error/replay segment without a CSRF token, and per-IP rate-limiting either blocks legitimate error bursts or sets limits so generous they don't protect anything. Defense in depth is:
...
| Response | Meaning |
|---|---|
| **204 No Content** | Sentry DSN unset — handler short-circuits. Set `NEXT_PUBLIC_SENTRY_DSN` if you want events. |
| **200 OK** | Tunnel forwarded successfully. Check Sentry dashboard for the event. |
| **400 Bad Request** | Envelope DSN didn't match `*.ingest.sentry.io`. Body validation failed. |
| **403 Forbidden** | **Sentry rejected the upstream POST.** Check the response body for `with_reason: ProjectId` — if present, the DSN's project key is **disabled** in Sentry (see § E below). |
| **413 Payload Too Large** | Envelope exceeded 1 MB. Usually a runaway session replay. |
| **502 Bad Gateway** | Upstream Sentry ingestion error. Check Sentry status page. |
...
2. Subscribe to `applicationEventBus` events:
   - `CONSENT_GIVEN` (analytics or all) → `client.addIntegration(Sentry.replayIntegration(...))` AND set sample rates (`replaysSessionSampleRate: 0.1`, `replaysOnErrorSampleRate: 1.0`).
...
This is the Workstream B pattern. If you reintroduce `integrations: undefined` (the default), Replay will load unconditionally and the consent-gating gets bypassed.
...
6. **POST returns 403:** see § E.2 (disabled DSN key).
7. **POST returns 502:** Sentry upstream issue. Check status.sentry.io.
