...
| Directory           | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Sections/`         | Page sections (30+ components) with Factory pattern — including the 11-tool calculator suite (`MoneyJobsCalculator` is the /tools entry point) (`CompoundInterestCalculator` — shared by compound-interest/retirement/goal-savings — `EmergencyFundCalculator`, `TimeToTargetCalculator`, `IdleCashCalculator`, `InflationImpactCalculator`, `CurrencyDepreciationCalculator`, `CardFeesCalculator`, `AssetHistoryCalculator`, plus the Real Talk stack for `/learn` (`Lesson`/`LessonThreeBeat`, `TalkArc`, `TalkQuiz`, `LearnIndex`; all 7 talks live 2026-07-16)) |
...
| `Analytics/`        | Adelaide Market `/market` surface — `RegimeScore`, `SignalCard`, `SignalCardsGrid`, and related macro-regime components                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `ErrorBoundary/`    | `RouteGroupError`, `RouteGroupLoading`, `PageErrorBoundary`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **Asset budgets:** Turbopack-aware bundle-budget gate (`scripts/check-bundle-budget.mjs`, run via `pnpm check:budget` after `pnpm build`). Walks `apps/web/.next/static/chunks/*.{js,css}` post-build and fails CI on regression — caps: 650 KB peak asset, 4900 KB total JS (progressively recalibrated from 3700 — 2026-06-17→3950, 06-19→4096, 07-01→4200, 07-02→4800, 07-16→4900 for the 24 lazy per-locale talk i18n chunks — driven by the cinematic hero's GSAP/`three` deps plus the investor/tools/learn growth; `three` core is dynamic-imported via `useWebGLScene`, kept out of critical JS so peak stays <650, and since 2026-07-17 the hero renders a single static frame on software-GL rasterizers — the PSI TBT fix, do-not-regress entry in `implementation-notes.md`), 512 KB total CSS, 250 JS chunks (both recalibrated 2026-07-16). The script (`scripts/check-bundle-budget.mjs`) is authoritative for these caps. Replaced the original webpack-plugin enforcement (was inert under Turbopack — F1 audit 2026-05-08, M3 reworked 2026-05-09).
...
5. **Per-section:** `SectionErrorBoundary` (`lib/errors/SectionErrorBoundary.tsx`), plus the React-component wrapper `PageErrorBoundary` (`components/ErrorBoundary/`), for granular UI fallback within a route.
