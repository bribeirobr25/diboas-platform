...
22. **Permission pre-prompt.** No OS permission dialog (push, location, contacts, camera, biometrics, tracking) fires cold: a value-framed screen explains the concrete benefit first, the decline is one equal-weight tap, and a declined ask is never looped. No unrelated flow is gated on the permission. [UX-64]
23. **Time honesty.** Any flow longer than ~1 minute, or dependent on a third party (identity verification, a transfer settling, a review queue), states its honest expected duration before the person commits — steps and minutes for in-flow work, days for reviews. An unstated duration is a trust breach, not a simplification. [UX-65, UX-44]
24. **Equal-weight decline.** Every optional ask (upsell, permission, sync, referral, upgrade) carries a real decline button of equal visual weight, worded neutrally ("Not now" — never self-shaming), costing one tap. A decline rendered as a text link, low-contrast, or below the fold is a FAIL here; a decline worded to shame is veto row 10 territory. [UX-66]
25. **Unbundled consent.** Consents with different purposes never share a checkbox or a button; marketing consent is always its own unchecked box; no box is ever pre-ticked; a consent that authorizes a money movement, a third-party data pull, or a standing permission is its own explicit act, not a line in a bundle. [UX-67]
26. **Whole-truth feed.** Security-relevant account events (step-up verifications, locks/unlocks, permission or signing-policy changes, new-device access) appear as user-visible activity entries; failed and canceled transactions remain in history, honestly labelled — nothing is silently disappeared or rewritten. [UX-68]
