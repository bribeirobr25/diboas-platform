...
For diBoaS this is free: our numbers are already real. Say "0.48%, no cap", not "low fees". Say what the pass-through actually costs. The precision _is_ the trust signal.
...
_Production reference (Mobbin shell study 2026-07-30, benchmark doc §4) — the limit stated before the wall:_ N26's add-money review states the remaining allowance in place ("You can add €4,980.00 more this month (€5,000.00 limit)"); bunq states the free-tier bound at the ask ("no extra cost — up to €500.00 every month"). Disclosing the constraint _before_ the user hits it converts an error state into an informed decision — proactive disclosure applied to limits, not only downsides.
...
_Added 2026-07-30 (founder-ratified) from the five-app onboarding benchmark formerly at `docs/sandbox-app/ONBOARDING_BENCHMARKS_MOBBIN_2026-07-30.md` (Revolut, Nubank MX, Stripe Dashboard, Coinbase, Cash App — Mobbin iOS captures), which was **removed** under the delete-after-execution policy once its substance was lifted into this canon — the benchmark's findings ARE UX-64…67 below plus checklist Part-3 rows 22–25, so this document remains self-contained. The benchmark validated the existing canon (the reference apps' strongest moves were already UX-34/37/39/44/46/52/56; several of their tactics already sit on the veto list) and surfaced four gaps — all on surfaces the marketing site never had and the sandbox app is about to have. This Part unpacks the C-6 parking lot (Appendix C) into canon; the remaining C-6 items (progressive disclosure, pyramid of trust, aha-fast, recovery UX) stay CTO-cluster input until their surfaces exist. Evidence below is observed shipped design from named products — no conversion statistics are asserted._
