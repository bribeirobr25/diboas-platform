...
- Test interactive states: hover (`browser_hover`), click (`browser_click`),
  type in inputs (`browser_type`), keyboard navigation (`browser_press_key`)
- Verify destructive actions have confirmations
- Assess perceived performance and responsiveness
- Take screenshots of each key state (`browser_take_screenshot`)

#### Phase 2: Responsiveness testing
- Desktop viewport (1440px): `browser_resize` then full screenshot
- Tablet viewport (768px): `browser_resize` then verify layout adapts properly
- Mobile viewport (375px): `browser_resize` then verify touch optimization
