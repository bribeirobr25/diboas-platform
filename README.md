...
- **Testing & CI:** strict TypeScript, **3,036 tests / 253 files (Vitest), measured 2026-09-22 after the Stage H visual gate (`5.309` resolved · source/class-aware freshness + current-facing availability enforcement)** across the workspace (web 1,606/132 · sandbox 1,042/96 · banking 118/4 · investing 86/5 · defi 184/16 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
