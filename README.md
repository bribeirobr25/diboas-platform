...
- **Testing & CI:** strict TypeScript, **3,065 tests / 255 files (Vitest), measured 2026-09-22 at Stage H closure (`5.309` + `5.436` resolved · source/class-aware freshness, current-facing availability on both the cost and rate consumers)** across the workspace (web 1,606/132 · sandbox 1,065/97 · banking 118/4 · investing 86/5 · defi 190/17 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
