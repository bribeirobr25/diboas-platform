...
- **Testing & CI:** strict TypeScript, **2,978 tests / 248 files (Vitest), measured 2026-09-21 after `5.431` (the system gate emits every authoritative front)** across the workspace (web 1,606/132 · sandbox 1,025/95 · banking 118/4 · investing 86/5 · defi 143/12 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
