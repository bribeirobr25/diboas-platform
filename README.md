...
- **Testing & CI:** strict TypeScript, **3,259 tests / 270 files (Vitest), measured 2026-09-24 after S1 of the 5.444 lane (a catalogue evidence requirement typed by each leg's return mechanism; not yet consumed)** across the workspace (web 1,606/132 · sandbox 1,109/100 · banking 126/5 · investing 86/5 · defi 332/28 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
