...
- **Testing & CI:** strict TypeScript, **3,296 tests / 275 files (Vitest), measured 2026-09-24 after the 5.444 lane's §16 observability completion (the typed catalogue decision and the refusal that names its leg are now recorded, not silent)** across the workspace (web 1,606/132 · sandbox 1,119/101 · banking 126/5 · investing 86/5 · defi 359/32 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
