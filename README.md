...
- **Testing & CI:** strict TypeScript, **3,152 tests / 259 files (Vitest), measured 2026-09-22 after the provider-safety block (kill switch · fallback eligibility · `5.440` provider-name liveness removed)** across the workspace (web 1,606/132 · sandbox 1,077/97 · banking 126/5 · investing 86/5 · defi 257/20 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
