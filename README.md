...
- **Testing & CI:** strict TypeScript, **3,230 tests / 266 files (Vitest), measured 2026-09-24 after Block D of the ratified market-evidence plan (retention per class; replay coverage is not a TTL; persistence gated on declared source rights)** across the workspace (web 1,606/132 · sandbox 1,109/100 · banking 126/5 · investing 86/5 · defi 303/24 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
