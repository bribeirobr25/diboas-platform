...
- **Testing & CI:** strict TypeScript, **3,284 tests / 274 files (Vitest), measured 2026-09-24 after S3 of the 5.444 lane (a leg is asked for what its return mechanism produces, so a strategy is no longer made unavailable by a rate no leg of it owes)** across the workspace (web 1,606/132 · sandbox 1,119/101 · banking 126/5 · investing 86/5 · defi 347/31 — tests/files per package, five per-package runs, none carried; both columns sum to the totals, checked by `review:counts`), Lighthouse CI + pa11y (WCAG 2.1 AA), and 6 GitHub Actions workflows (CI, security audit, accessibility, E2E, Lighthouse, quarterly security scan) plus a weekly /market data-refresh workflow.
