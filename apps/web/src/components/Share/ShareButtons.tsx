 ...
 *
 * Displays a row of share buttons for different platforms
 * Supports both icon-only and icon+text modes
 * Respects market-specific platform priority
 */
...
interface ShareButtonsProps {
  /** Callback when a platform button is clicked */
  onShare: (platform: SharePlatform) => void;
  /** Platforms to display (defaults to locale-specific platforms) */
  platforms?: SharePlatform[];
  /** Current locale for market-specific ordering */
  locale?: CardLocale;
...
/**
 * Get platforms ordered by locale-specific priority
 */
function getLocalePlatforms(locale: CardLocale): SharePlatform[] {
  const priority = PLATFORM_PRIORITY_BY_LOCALE[locale];
  if (priority) {
    return priority as SharePlatform[];
  }
...
const PLATFORM_ICONS: Record<SharePlatform, React.FC> = {
...
export function ShareButtons({
  ...
  const orderedPlatforms = useMemo(() => {
    ...
    return getLocalePlatforms(locale);
  }, [platforms, locale]);

  const getLabel = (platform: SharePlatform): string => {
    return intl.formatMessage({ id: `share.platform.${platform}` });
  };
