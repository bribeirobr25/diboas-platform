...
export function PathSelectorScreen() {
  const intl = useIntl();
  const { selectPath, previousScreen } = useDreamMode();
        ...
        <div className={styles.pathGrid}>
          {PATH_ORDER.map((pathId, index) => {
            ...
            return (
              ...
              >
                <span className={styles.pathIcon}>{config.icon}</span>
                <h3 className={styles.pathName}>
                  {intl.formatMessage({ id: `dreamMode.paths.${pathId}.name` })}
                </h3>
                ...
                <div className={styles.pathRisk}>
                  ...
                  {config.warning && (
                    <span className={styles.pathWarning}>⚠️</span>
                  )}
                </div>
              </button>
            );
