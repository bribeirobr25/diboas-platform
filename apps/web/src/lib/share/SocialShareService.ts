...
export class SocialShareService {
  ...
  /**
   * Wrap a platform share handler with tracking
   */
  private wrapShare(platform: SharePlatform, result: ShareResult): ShareResult {
    if (result.success) {
      this.track({ platform, cardType: this.cardType, locale: this.locale });
    }
    return result;
  }

  shareToTwitter(content: ShareContent): ShareResult {
    return this.wrapShare('twitter', shareToTwitter(content, this.locale, this.cardType, this.referralCode));
  }

  shareToWhatsApp(content: ShareContent): ShareResult {
    return this.wrapShare('whatsapp', shareToWhatsApp(content, this.cardType, this.referralCode));
  }

  shareToFacebook(content: ShareContent): ShareResult {
    return this.wrapShare('facebook', shareToFacebook(content, this.cardType, this.referralCode));
  }

  shareToLinkedIn(content: ShareContent): ShareResult {
    return this.wrapShare('linkedin', shareToLinkedIn(content, this.cardType, this.referralCode));
  }

  shareToTelegram(content: ShareContent): ShareResult {
    return this.wrapShare('telegram', shareToTelegram(content, this.cardType, this.referralCode));
  }
  ...
  async share(platform: SharePlatform, content: ShareContent): Promise<ShareResult> {
    ...
    const eventType = result.success
      ? ApplicationEventType.SHARE_COMPLETED
      : result.cancelled
        ? ApplicationEventType.SHARE_CANCELLED
        : ApplicationEventType.SHARE_FAILED;
