...
function createThreshold(
  ...
  if (good >= poor) {
    throw new Error(
      `Good threshold (${good}) must be less than poor threshold (${poor}) for ${description}`
    );
  }
...
function createReverseThreshold(
  ...
  if (good <= poor) {
    throw new Error(
      `Good threshold (${good}) must be greater than poor threshold (${poor}) for ${description}`
    );
  }
...
export const WEB_VITALS_THRESHOLDS: WebVitalsThresholds = {
  ...
  TTFB: createThreshold(800, 1800, 'ms', 'Time to First Byte - Server response time'),
  INP: createThreshold(
    200,
    500,
    'ms',
...
export const API_PERFORMANCE_THRESHOLDS: ApiPerformanceThresholds = {
  ...
  errorRate: createThreshold(0.1, 1.0, '%', 'API error rate percentage'),
  throughput: createReverseThreshold(1000, 100, 'req/s', 'API requests per second capacity'),
  availability: createReverseThreshold(99.9, 99.0, '%', 'API uptime percentage'),
};
...
export const SYSTEM_PERFORMANCE_THRESHOLDS: SystemPerformanceThresholds = {
  memoryUsage: createThreshold(70, 90, '%', 'System memory utilization percentage'),
  cpuUsage: createThreshold(60, 85, '%', 'System CPU utilization percentage'),
  diskUsage: createThreshold(75, 90, '%', 'Disk space utilization percentage'),
  networkLatency: createThreshold(50, 200, 'ms', 'Network round-trip latency'),
};
...
export const BUNDLE_SIZE_THRESHOLDS = {
  javascript: createThreshold(250, 500, 'KB', 'JavaScript bundle size'),
  css: createThreshold(50, 100, 'KB', 'CSS bundle size'),
  images: createThreshold(500, 1000, 'KB', 'Total image size per page'),
  fonts: createThreshold(100, 200, 'KB', 'Font files total size'),
} as const;
...
export const BUSINESS_PERFORMANCE_THRESHOLDS = {
  transactionProcessing: createThreshold(2000, 5000, 'ms', 'Financial transaction processing time'),
  portfolioCalculation: createThreshold(1000, 3000, 'ms', 'Portfolio value calculation time'),
  authenticationTime: createThreshold(500, 2000, 'ms', 'User authentication response time'),
  dataSync: createThreshold(5000, 15000, 'ms', 'Financial data synchronization time'),
} as const;
